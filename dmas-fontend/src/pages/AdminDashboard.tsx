...
const AdminDashboard = () => {
  ...
  const handlePublish = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // This calls your @PutMapping("/{id}/verify") endpoint
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-2xl font-black mb-8">Admin Control Center</h1>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-slate-200">
            ...
            <tbody className="divide-y divide-slate-100">
              {alerts.map((alert) => (
                ...
                >
                  ...
                  <td className="p-4">
                    {alert.verified ? (
                      <span className="text-green-600 bg-green-50 px-2 py-1 rounded text-[10px] font-bold">
                        LIVE
                      </span>
                    ) : (
                      <span className="text-amber-600 bg-amber-50 px-2 py-1 rounded text-[10px] font-bold">
                        PENDING
                      </span>
                    )}
                  </td>
                  <td className="p-4 text-right">
                    {!alert.verified && (
                      <button
                        onClick={() => handleVerify(alert.id, alert)}
                        className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded-lg text-xs font-bold transition-all shadow-lg shadow-blue-200"
                      >
                        Approve & Broadcast
                      </button>
                    )}
          ...
          {selectedAlert && (
            <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
              <div className="bg-white rounded-2xl p-8 max-w-2xl w-full shadow-2xl border border-slate-200">
                ...
                <form onSubmit={handlePublish} className="space-y-4">
                  ...
                  <div className="flex justify-end gap-3 mt-6">
                    <button
                      ...
                      className="px-6 py-2 text-slate-500 font-bold hover:bg-slate-100 rounded-lg"
                    >
                      Cancel
                    </button>
                    <button
